#!/usr/bin/env python3
"""
Atualiza tickets de [Matemática] - Ano 4 no GitHub Projects.
...
Uso:
    python3 update_matematica4_tickets.py           # dry-run
    python3 update_matematica4_tickets.py --apply   # aplica as atualizações
"""
...
def parse_reference() -> dict[str, ReferenceTicket]:
    ...
    for match in pattern.finditer(text):
        ...
        title = f"[Matemática] - Ano 4 - {code} {match.group(2).strip()}"
        body  = f"# Description\n\n{match.group(3).strip()}"
...
def issue_code(title: str) -> str | None:
    match = re.match(r"^\[Matemática\] - Ano 4 -\s*(\d+(?:\.\d+)?)", title, re.IGNORECASE)
...
def fetch_project_issues(env: dict) -> list[ProjectIssue]:
    ...
    # Filtra apenas tickets do Ano 4
    ano4 = [i for i in issues if re.match(r"^\[Matemática\] - Ano 4", i.title, re.IGNORECASE)]
    print(f"Tickets do Ano 4 encontrados no Project: {len(ano4)}")
    return ano4
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Atualiza tickets de Matemática Ano 4 no Kanban Bibline."
    )
