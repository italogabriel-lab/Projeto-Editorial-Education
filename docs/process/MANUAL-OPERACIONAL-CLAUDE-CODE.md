# Manual operacional do framework editorial no Claude Code

Data de referência, 23 de abril de 2026.

Este documento é o mapa operacional do framework editorial deste repositório. O objetivo é permitir que qualquer pessoa nova na equipe entenda:
...
- como acionar cada peça no Claude Code
...
1. `CLAUDE.md` e `AGENTS.md` definem as regras globais do ambiente.
2. `.claude/` organiza comandos, regras, skills e personas para uso diário no Claude Code.
...
- `commands/` contém os comandos operacionais usados no Claude Code
...
|---|---|---|---|---|
| 0 | Orchestrator | pedido do usuário | diagnóstico e roteamento | `.claude/commands/orchestrator.md` |
| 1 | Researcher | tema, ano, aula | relatório de pesquisa | `.claude/commands/researcher.md` |
| 2 | Writer | pesquisa + plano pedagógico | rascunho da aula | `.claude/commands/writer.md` |
| 3 | Standardizer | rascunho | aula em Rise Blocks | `.claude/commands/standardizer.md` |
| 4 | Reviewer | aula formatada | aprovação ou rejeição | `.claude/commands/reviewer.md` |
| 5 | Copywriter | aula aprovada | versão polida | `.claude/commands/copywriter.md` |
| 6 | Registro | aula final | macro, matriz, visão e revisão alinhados | `trivium-method-editorial/scripts/sync_titles.py` |
| 7 | Publisher | aula final pronta | arquivo salvo e publicado | `.claude/commands/publisher.md` |
...
## 5. Como chamar no Claude Code

Há duas formas práticas de uso.

### 5.1 Modo recomendado para a equipe

Use linguagem natural citando claramente o agente ou o workflow.
...
Esse é o modo mais seguro porque funciona mesmo quando a interface do Claude Code não expõe slash commands customizados.

### 5.2 Modo slash command

Quando o ambiente do Claude Code reconhecer os comandos markdown de `.claude/commands/`, a equipe pode usar chamadas curtas como:
...
- `/researcher pesquise a aula 26.2 do 3º ano`
- `/writer redija a aula 26.2 do 3º ano`
- `/reviewer revise a aula 26.2`
- `/devops faça commit e push`
...
|---|---|---|
| `editorial-orchestrator` | `.claude/agents/editorial-orchestrator.md` | coordenação editorial e priorização |
| `publishing-operator` | `.claude/agents/publishing-operator.md` | apoio à publicação e operação de saída |
| `qa-reviewer` | `.claude/agents/qa-reviewer.md` | reforço de QA e revisão final |
...
```text
CLAUDE.md
  -> carrega regras de .claude/rules/
  -> orienta uso de .claude/commands/
...
1. `AGENTS.md`
2. `CLAUDE.md`
3. `docs/process/MANUAL-OPERACIONAL-CLAUDE-CODE.md`
...
Alguns documentos antigos ainda citam caminhos como `editorial-squad/` ou `.agent/`.
O estado operacional atual do repositório usa:
...
Se houver divergência entre documentação antiga e estrutura atual, a estrutura atual prevalece.
...
## 16. Comandos prontos para copiar no Claude Code
