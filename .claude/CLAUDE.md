...
- Manter o Accordion do Definir com tema ou pergunta simples, `@link_png@`, MP3 com definição curta, explicação completa e conexão teológica na mesma linha, separadas por espaço, e texto visual equivalente após `[MP3\]`. Aplicar a mesma linha única ao áudio do Narrar.
- Usar `#VOX:` como marcador literal dentro de todos os blocos `[MP3/]`. Não usar `#FSH:` nem voice IDs antigos.
- Em aula regular com um único hotspot no `[+IMAGE_LABELED]`, usar a coordenada central `49 50`. Revisões `.4` podem ter vários hotspots com coordenadas próprias.
- Usar os enunciados fixos dos hábitos: em Definir, "Leia o fato e ouça o áudio clicando abaixo." antes do Accordion, ou dos TABS no 2º ano; em Recordar, "Ouça e repita o fato abaixo."; em Praticar, antes do Fill_In, "Complete o fato abaixo com a palavra correta.".
- No Recordar da revisão semanal `.4`, usar literalmente "Recorde o fato estudado durante a semana.".
- Tratar a **semana como unidade pedagógica**: definição curta única em `x.1`, `x.2` e `x.3`; termo principal compartilhado; conexão teológica idêntica nas três aulas; música ou rima comum no Recordar. Só variam a explicação da palavra-chave, as imagens e a Atividade Extra.
- **Conexão teológica semanal**: no início do Definir, usar a ordem definição curta, explicação variável da palavra-chave e conexão teológica idêntica na semana. A chamada "Veja o vídeo abaixo." fica na mesma linha da conexão.
- **Definir do 2º ano**: usar um único `[+TABS]` no lugar de `[+ACCORDION]`, com título, `@link_png@`, definição, explicação, áudio, texto visual e conexão teológica, seguido diretamente de `[-TABS]`.
- **Narrar = espelho literal do Definir**: o bloco de áudio e o texto visual do `[+IMAGE_TEXT_ASIDE]` no Narrar são literalmente idênticos ao conteúdo do Definir daquela aula, incluindo definição curta, explicação e conexão teológica. Antes de `[MP3\]`, texto plain (sem negrito). Após `[MP3\]`, texto com negritos progressivos, igual ao texto visual do Accordion. Nenhuma frase adicional é permitida.
