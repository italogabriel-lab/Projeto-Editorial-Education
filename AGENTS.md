...
- Em `x.2` e `x.3`, só variam a explicação da palavra-chave, as imagens e a Atividade Extra. A conexão teológica permanece idêntica nas três aulas da semana.
- **Conexão teológica semanal**: no `[+PARAGRAPH]` inicial do Definir, manter a ordem definição curta, explicação variável da palavra-chave e conexão teológica idêntica na semana. A chamada "Veja o vídeo abaixo." deve ficar na mesma linha da conexão teológica. A conexão deve estar ligada ao tema da semana e ser repetida literalmente em `x.1`, `x.2` e `x.3`.
- **Bloco de definição do 2º ano**: usar um único `[+TABS]` no lugar de `[+ACCORDION]`, contendo título, `@link_png@`, definição, explicação, áudio, texto visual e conexão teológica, seguido diretamente de `[-TABS]`. Nos demais anos, seguir o bloco previsto pelo template específico.
- **Narrar = espelho literal do Definir**: o bloco `[+IMAGE_TEXT_ASIDE]` do Narrar repete literalmente a definição curta, a explicação da palavra-chave e a conexão teológica do Definir daquela aula. Antes de `[MP3\]`, texto plain (sem negrito). Após `[MP3\]`, texto com negritos progressivos idênticos ao Accordion. Nenhuma frase adicional é permitida.
- **Perguntas do Narrar**: no 2º ano, usar exatamente 2 perguntas direcionadas por aula, heading `Perguntas` (plural). Ambas devem ser derivadas diretamente do vocabulário do texto do Narrar e ter respostas explícitas na definição, na explicação ou na conexão teológica. Nos demais padrões, seguir a quantidade definida pelo template do ano. Proibido: "O que você percebeu sobre...", perguntas abertas, reflexivas, abstratas ou inferenciais.
...
- Accordion ou TABS do Definir: tema ou pergunta simples, `@link_png@`, MP3 e texto visual. No 2º ano, usar TABS.
- MP3 do Accordion ou do TABS do 2º ano: narrar a definição curta, a explicação completa da palavra-chave e a conexão teológica em texto narrável.
- **Áudio em linha única**: quando o MP3 contiver definição curta, explicação e conexão teológica, os três trechos ficam na mesma linha, separados por espaço. Aplicar no Accordion, no TABS do 2º ano e no Narrar. O texto visual após `[MP3\]` pode permanecer em parágrafos, com negritos.
- **Marcador padrão de MP3**: em todas as aulas, usar a linha literal `#VOX:` dentro de `[MP3/]`, sem o identificador antigo `#FSH:` ou outro voice ID.
- **Coordenada central única**: quando a aula regular usar apenas um hotspot em `[+IMAGE_LABELED]`, usar a coordenada `49 50`. Revisões `.4` com vários hotspots mantêm coordenadas próprias.
- **Enunciados fixos dos hábitos**: antes do Accordion ou dos TABS em Definir, usar literalmente "Leia o fato e ouça o áudio clicando abaixo.". No primeiro parágrafo de Recordar, usar literalmente "Ouça e repita o fato abaixo.". No primeiro parágrafo de Praticar, antes do Fill_In, usar literalmente "Complete o fato abaixo com a palavra correta.".
...
- **Recordar da revisão semanal `.4`**: o parágrafo de abertura usa literalmente "Recorde o fato estudado durante a semana.".
